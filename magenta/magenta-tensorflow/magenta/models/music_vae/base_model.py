...
class MusicVAE(object):
  ...
  def get_attention(self, rsequence):
    attention_size = 128
    hidden_size = rsequence.shape[2].value
    ...
    with tf.name_scope('v'):
      v = tf.tanh(tf.tensordot(rsequence, w_omega, axes=1) + b_omega)
    
    vu = tf.tensordot(v, u_omega, axes=1, name='vu')
    alphas = tf.nn.softmax(vu, name='alphas')
    output = tf.reduce_sum(rsequence*tf.expand_dims(alphas, -1), 1)
    return output

  def encode(self, sequence, rsequence, sequence_length, control_sequence=None):
    ...
    iencoder_output = self.encoder.encode(sequence, sequence_length)
    rencoder_output = self.encoder.encode(rsequence, sequence_length)
    encoder_output = tf.add(iencoder_output, rencoder_output)
  ...
  def _compute_model_loss(
      self, input_sequence, input_rsequence, output_sequence, sequence_length, osequence_length, control_sequence):
    """Builds a model with loss for train/eval."""
    hparams = self.hparams
    batch_size = hparams.batch_size

    input_sequence = tf.to_float(input_sequence)
    input_rsequence = tf.to_float(input_rsequence)
    output_sequence = tf.to_float(output_sequence)

    max_seq_len = tf.minimum(tf.shape(input_sequence)[1], hparams.max_seq_len)
    max_oseq_len = tf.minimum(tf.shape(output_sequence)[1], hparams.max_oseq_len)

    input_sequence = input_sequence[:, :max_seq_len]
    input_rsequence = input_rsequence[:, :max_seq_len]
    ...
    x_target = output_sequence[:, :max_oseq_len]
    # Inputs to be fed to decoder, including zero padding for the initial input.
    x_input = tf.pad(output_sequence[:, :max_oseq_len - 1],
                     [(0, 0), (1, 0), (0, 0)])
    x_length = tf.minimum(sequence_length, max_seq_len)
    ox_length = tf.minimum(osequence_length, max_oseq_len)

    # Either encode to get `z`, or do unconditional, decoder-only.
    if hparams.z_size:  # vae mode:
      q_z = self.encode(input_sequence, input_rsequence, x_length, control_sequence)
      z = q_z.sample()
    ...
    r_loss, metric_map = self.decoder.reconstruction_loss(
        x_input, x_target, ox_length, z, control_sequence)[0:2]
  ...
  def train(self, input_sequence, input_rsequence, output_sequence, sequence_length,
            osequence_length, control_sequence=None):
    ...
    _, scalars_to_summarize = self._compute_model_loss(
        input_sequence, input_rsequence, output_sequence, sequence_length, osequence_length, control_sequence)
  ...
  def eval(self, input_sequence, input_rsequence, output_sequence, sequence_length,
           osequence_length, control_sequence=None):
