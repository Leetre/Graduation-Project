...
from magenta.models.music_vae.data_utils import decode_example
...
class NoteSequenceAugmenter(object):
  ...
  def get_all(self, note_sequence):
    ...
    for t in range(transpose_min, transpose_max + 1):
      yield sequences_lib.augment_note_sequence(
        note_sequence,
        stretch_min,
        stretch_max,
        t,
...
class TrioConverter(BaseNoteSequenceConverter):
  ...
  def to_tensors(self, note_sequence):
    results = super(TrioConverter, self).to_tensors(note_sequence)
    if results.inputs:
      # print(np.split(results.inputs[0], [90, 180, 692], 0))
      inputs = np.array(
        [[x2.argmax(-1) for x2 in np.split(x, [90, 180], 1)] for x in results.inputs])
      results = ConverterTensors(inputs=inputs, outputs=inputs, controls=results.controls, lengths=results.lengths)
    return results
...
def get_dataset(
  ...
  dataset = dataset.map(decode_example({
    'notes': tf.VarLenFeature(tf.int64),
    'chords': tf.VarLenFeature(tf.int64),
    "shape": tf.VarLenFeature(tf.int64),
    "lengths": tf.VarLenFeature(tf.int64),
  }), num_parallel_calls=num_threads)

  from magenta.models.music_vae.data_hierarchical import MultiInstrumentPerformanceConverter
  if isinstance(data_converter, MultiInstrumentPerformanceConverter):
    def _preprocess(example):  # one measure only now
      shape = example['shape'][1:]
      lengths = tf.to_int32(example['lengths'])  # [n_instru]

      max_e_per_instru = config.data_converter._max_events_per_instrument
      max_instru = config.data_converter._max_num_instruments

      def reshape_pad_and_onehot(x, shape, depth):
        x = tf.reshape(x, shape)  # [n_instru, n_notes]
        delta = max_e_per_instru - shape[1]
        x = tf.cond(tf.greater(delta, 0),
                    lambda: tf.concat([x, tf.zeros([shape[0], delta], tf.int64)], 1),
                    lambda: x)
        delta = max_instru - shape[0]
        x = tf.cond(tf.greater(delta, 0),
                    lambda: tf.concat([x, tf.zeros([delta, max_e_per_instru], tf.int64)], 0),
                    lambda: x)
        x.set_shape([None, None])
        x = tf.reshape(x, [-1])
        x = tf.one_hot(x, depth, on_value=1.0, off_value=0.0, axis=-1)  # [n_instru_max * n_notes_max, depth]
        return x

      notes = reshape_pad_and_onehot(example['notes'], shape, config.data_converter.input_depth)
      chords = reshape_pad_and_onehot(example['chords'], shape, config.data_converter.control_depth) \
        if config.data_converter.control_depth else tf.ones([1])
      return notes, notes, chords, lengths
  elif isinstance(data_converter, TrioConverter):
    def _preprocess(example):  # one measure only now
      shape = example['shape']
      notes = example['notes']
      notes = tf.reshape(notes, shape)
      notes.set_shape([3, None])
      x = tf.concat([
        tf.one_hot(notes[0], 90, on_value=1.0, off_value=0.0, axis=-1),
        tf.one_hot(notes[1], 90, on_value=1.0, off_value=0.0, axis=-1),
        tf.one_hot(notes[2], 512, on_value=1.0, off_value=0.0, axis=-1),
      ], -1)
      return x, x, tf.ones([1]), tf.ones([4], tf.int32) * 16  # TODO, length
