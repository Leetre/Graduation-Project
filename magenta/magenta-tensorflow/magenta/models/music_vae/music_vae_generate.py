...
flags.DEFINE_string(
    'mode', 'sample',
    'Generate mode (`sample` or `interpolate` or `generate`).')
flags.DEFINE_string(
    'input_midi', None,
    'Path of MIDI file for generating.')
flags.DEFINE_string(
    'input_midi_1', None,
...
def run(config_map):
  ...
  if FLAGS.mode != 'sample' and FLAGS.mode != 'interpolate' and FLAGS.mode != 'generate':
  ...
  if FLAGS.mode == 'generate':
    if FLAGS.input_midi is None:
      raise ValueError(
          '`--input_midi` must be specified in '
          '`generate` mode.')
    input_midi = os.path.expanduser(FLAGS.input_midi)
    if not os.path.exists(input_midi):
      raise ValueError('Input MIDI not found: %s' % FLAGS.input_midi)
    input_m = mm.midi_file_to_note_sequence(input_midi)
    ...
    #     FLAGS.config)
    # _check_extract_examples(input_m, FLAGS.input_midi, 1)
  ...
  if FLAGS.mode == 'interpolate':
    ...
    results = model.sample(
        n=FLAGS.num_outputs,
        length=config.hparams.max_oseq_len,
        temperature=FLAGS.temperature)
  elif FLAGS.mode == 'generate':
    logging.info('Generating...')
    z, _, _ = model.encode(note_sequences=[input_m])
    results = model.decode(
        length=config.hparams.max_oseq_len,
        z=z,
        temperature=FLAGS.temperature)
