...
class TrainedModel(object):
  ...
  def __init__(self, config, batch_size, checkpoint_dir_or_path=None,
    ...
    with tf.Graph().as_default():
      ...
      self._outputs, self._decoder_results = model.sample(
          ...
          **sample_kwargs)
      if self._config.hparams.z_size:
        q_z = model.encode(self._inputs, self._rinputs, self._inputs_length, self._controls)
        self._mu = q_z.loc
  ...
  def encode(self, note_sequences, assert_same_length=False):
    ...
    for note_sequence in note_sequences:
      ...
      ishape = list(extracted_tensors.inputs.shape)
  ...
  def mask_a(self, a, max_length, depth, mode):
    if mode == 1:
      a_pre = a[0:2*max_length//4, 0:depth]
      a_post = a[3*max_length//4:max_length, 0:depth]
      zero_a = np.zeros([max_length//4, depth])
      result = np.concatenate((a_pre, zero_a, a_post), axis=0)
      return result
    else:
      pre_zero = np.zeros([2*max_length//4, depth])
      post_zero = np.zeros([max_length//4, depth])
      middle = a[2*max_length//4:3*max_length//4, 0:depth]
      result = np.concatenate((pre_zero, middle, post_zero), axis=0)
      return result

  def convert_to_one_hot(self, y, c):
    return np.eye(c)[y.reshape(-1)]

  def encode_tensors(self, input_tensors, lengths, mode=1, control_tensors=None):
    ...
    length_array = np.pad(
        ...
        'constant')
    max_length = max([len(t) for t in input_tensors[0]])
    # inputs_array = np.zeros(
    #     [len(input_tensors), max_length, input_depth])
    # for i, t in enumerate(input_tensors[0]):
    #   inputs_array[i, :len(t)] = t
    inputs_array = []
    rinputs_array = []
    for i in range(n):
      tmp_inputs_array = np.concatenate((
        self.convert_to_one_hot(input_tensors[i][0], 90),
        self.convert_to_one_hot(input_tensors[i][1], 90),
        self.convert_to_one_hot(input_tensors[i][2], 512),
      ), -1)
      tmp_inputs_array = np.array(self.mask_a(tmp_inputs_array, max_length, input_depth, 1))
      tmp_rinputs_array = np.array(self.mask_a(tmp_inputs_array, max_length, input_depth, 2))
      inputs_array.append(tmp_inputs_array)
      rinputs_array.append(tmp_rinputs_array)
    inputs_array = np.array(inputs_array)
    rinputs_array = np.array(rinputs_array)
    ...
    for i in range(len(inputs_array) // batch_size):
      ...
      feed_dict = {self._inputs: inputs_array[batch_begin:batch_end],
                   self._rinputs: rinputs_array[batch_begin:batch_end],
