...
from magenta.models.music_vae.data import ConverterTensors
...
class BaseHierarchicalConverter(data.BaseConverter):
  ...
  #   if not self._max_lengths: # max_lengths = [_max_num_chunks, max_num_instruments, max_events_per_instrument]
  ...
  #         data.np_onehot([self.end_token], self.input_depth))) # [n_chunks * n_track, T, depth]
  ...
  #       [pad_with_value(x, self._max_lengths[-1], 0) for x in input_])  # [n_chunks * n_track * max_T, depth]
...
class MultiInstrumentPerformanceConverter(
  ...
  def to_tensors(self, note_sequence):
    """Python method that converts `note_sequence` into list of tensors."""
    results = super(MultiInstrumentPerformanceConverter, self).to_tensors(note_sequence)
    inputs = []
    controls = []
    lengths = []
    for d, c in zip(results.inputs, results.controls):
      max_len = max([len(x1) for x in d for x1 in x])
      l = [len(l2) for l1 in d for l2 in l1]  # [n_bars, n_instr]

      d = np.array([[np.pad(l2, [0, max_len - len(l2)], 'constant', constant_values=0) for l2 in l1] for l1 in
                    d])  # [n_bars, n_instr, n_notes]
      if self._chord_encoding is not None:
        c = np.array(
          [[np.pad(l2, [0, max_len - len(l2)], 'constant', constant_values=0) for l2 in l1] for l1 in c])
      else:
        c = np.array([0])
      inputs.append(d)
      controls.append(c)
    ...
    return ConverterTensors(inputs=inputs, outputs=inputs, controls=controls, lengths=lengths)
  ...
  def _quantized_subsequence_to_tensors(self, quantized_subsequence):
    ...
    tracks, stats = performance_pipeline.extract_performances(
        quantized_subsequence,
        max_steps_truncate=self._max_steps_truncate,
        num_velocity_bins=self._num_velocity_bins,
        split_instruments=self._max_num_instruments > 1)
    ...
    for track in tracks:
      ...
      if not all(l <= self._max_events_per_instrument - 2
        ...
        # print("length error", track_chunk_lengths)
        return [], []
    ...
    if total_length < self._min_total_events:
      # print("total_length error", total_length)
      return [], []
    ...
    for chunk_tracks in chunks:
      ...
      for track in chunk_tracks:
        ...
        track_tensors.append(track_tokens)
      ...
      if self._chord_encoding:
        ...
        try:
          ...
          for chords in track_chords:
            ...
            track_chord_tensors.append(track_chord_tokens)
    ...
    return chunk_tensors, chunk_chord_tensors  # (n_chunks, n_tracks, n_events, 490), (n_chunks, n_tracks, n_events, 49)
  ...
  def _to_tensors(self, note_sequence):
    ...
    for subsequence in subsequences:
      ...
      tensors, chord_tensors = self._quantized_subsequence_to_tensors(
        quantized_subsequence)  # (n_chunks, n_tracks, n_events, 490), (n_chunks, n_tracks, n_events, 49)
      if tensors:
